...
public Plugin myinfo = 
{
    ...
    description = "Allows players to control bot_quota via chat commands",
...
public Action Command_SetBots(int client, int args)
{
    if (args != 1)
    {
        ReplyToCommand(client, "Usage: !bots <0-30>");
    ...
    {
        ReplyToCommand(client, "Bot quota must be between 0 and 30.");
        return Plugin_Handled;
    }
...
int GetBotQuota()
{
    return GetConVarInt(FindConVar("bot_quota"));
}

void SetBotQuota(int quota)
{
    SetConVarInt(FindConVar("bot_quota"), quota);
}