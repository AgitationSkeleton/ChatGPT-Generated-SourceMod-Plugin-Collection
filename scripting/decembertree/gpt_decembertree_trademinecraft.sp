/**
 * TF2 December Trees (secret_facility)
 * Spawns 3 Christmas tree prop_dynamics only during December (unless forced).
 *
 * Revision:
 * - Revert to fixed coordinates (no floor detection).
...
int gTreeEntRefs[3] = { INVALID_ENT_REFERENCE, INVALID_ENT_REFERENCE, INVALID_ENT_REFERENCE };
bool gDownloadsAdded = false;

public Plugin myinfo =
{
    name        = "TF2 December Trees (trade_minecraft_b38_mvm)",
    author      = "ChatGPT",
    description = "Spawns 3 Christmas tree props on trade_minecraft_b38_mvm during December (or forced) and adds required downloads.",
...
public void OnPluginStart()
{
    ...
    // This makes it easy to iterate on Z-level without changing maps.
    if (ShouldRunNow())
    {
...
bool ShouldRunNow()
{
    ...
    if (!StrEqual(mapName, "trade_minecraft_b38_mvm", false))
    {
        return false;
    }
...
void RemoveExistingTreesInWorld()
{
    ...
    for (int entIndex = MaxClients + 1; entIndex < maxEnts; entIndex++)
    {
        ...
        // (You asked for prop_dynamic specifically.)
...
{
    // Your provided coordinates (verbatim).
    float origins[3][3] =
    {
        { 2480.437500, 576.177002, 64.031311 },
        { 1031.525391, -829.364258, 64.031311 },
        { -435.162842, 579.257507, 64.031311 }
    };
