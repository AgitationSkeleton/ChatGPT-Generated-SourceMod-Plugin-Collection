...
public plugin_init()
{
    ...
    register_clcmd("aqs_test", "Cmd_TestAQS", ADMIN_KICK, "<1..12|suicide|teamkill|headshot> - test");
...
public plugin_cfg()
{
    gFirstBloodAvailable = true; // reset on map start
}

public client_connect(id)
{
    gKillChain[id] = 0;
    gLastKillTime[id] = 0.0;
}
...
stock handle_deathmsg(killer, victim, headshot, const weapon[])
{
    ...
    if (now - gLastKillTime[killer] <= win)
        gKillChain[killer]++;
    else
        gKillChain[killer] = 1;
    ...
    if (gFirstBloodAvailable)
    {
        ...
        if (get_pcvar_num(gCvarDebug))
            server_print("[AQS] First Blood by %d", killer);
        spk_first_blood();
    }
...
stock play_suicide_or_world(victim, const weapon[])
{
    ...
    if (get_pcvar_num(gCvarSounds))
        spk_all("AQS/vox/humiliation.wav");
}
...
stock play_teamkill(killer, victim, const weapon[])
{
    ...
    if (get_pcvar_num(gCvarSounds))
        spk_all("AQS/vox/teamkiller.wav");
}
...
stock announce_kill(killer, victim, const weapon[], chain, headshot)
{
    ...
    switch (chain) {
        ...
        case 8: copy(suffix, 39, " - WICKED SICK!");
        case 9: copy(suffix, 39, " - GODLIKE!");
        default: copy(suffix, 39, " - BEYOND GODLIKE!");
    }
...
/* Chain → sound. Headshot overrides if flagged. */
stock play_chain_sound(chain, headshot)
{
    if (!get_pcvar_num(gCvarSounds)) return;

    if (headshot > 0) {
        spk_rand_from_group(gHeadshots, sizeof gHeadshots);
        return;
    }

    if (chain <= 1) { spk_rand_from_group(gSingles, sizeof gSingles); return; }
    ...
    if (chain == 8) { spk_all("AQS/vox/whickedsick.wav"); return; }
    if (chain == 9) { spk_all("AQS/vox/godlike.wav"); return; }
...
/* First Blood */
stock spk_first_blood()
{
    spk_rand_from_group(gFirstBlood, sizeof gFirstBlood);
}

/* ---------- Utils ---------- */
stock spk_all(const soundpath[])
{
...
public Cmd_TestAQS(id, level, cid)
{
    ...
    if (equali(arg, "teamkill")) {
        new t = find_any_teammate(id);
        if (!t) t = id;
        play_teamkill(id, t, "disc");
    ...
    if (get_pcvar_num(gCvarDebug))
        server_print("[AQS] Test chain=%d", n);

    gKillChain[id] = n;
    gLastKillTime[id] = get_gametime();
...
stock find_any_teammate(id)
{
    ...
    for (i = 1; i <= MAX_PLAYERS; i++) {
        if (i == id) continue;
        if (is_user_connected(i) && get_user_team(i) == myteam)
            return i;
    }
    return 0;
}
